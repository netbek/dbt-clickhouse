{% macro clickhouse__batch_sequence_create_table(temporary, relation, sql) %}
  ...
  {%- set batch_relation = config.require('batch_relation') -%}
  {%- set batch_relation_alias = config.get('batch_relation_alias') -%}
  ...
  {% for offset in range(num_batches) -%}
    ...
    {%- set filtered_sql = clickhouse__get_batch_sequence_sql(sql, batch_column, batch_size, range_min, range_max, offset, relation_alias=batch_relation_alias) -%}
